...
$THIS_CISIS/mx $ALLFACIC btell=0 "$KEY=$" lw=99999 "proc='d920a920{',s('000000000000000000000000000000',v920),'{'"  "proc='d920a920{',mid(v920,size(v920)-30+1,30),'{'" "pft=replace(s(v920,'|',v911,'|',v912,'|',v913,'|',v914,'|',if instr(':PT:S:NE:BI:IN:AN:',s(':',v916,':'))>0 then v916 else '' fi,'|',v910,'|',if instr(':PT:S:NE:BI:IN:AN:',s(':',v916,':'))>0 then 'S' else 'R' fi),'| |','||')/" now | sort > $THIS_PROC_TMP_PATH/ORD.txt
fi

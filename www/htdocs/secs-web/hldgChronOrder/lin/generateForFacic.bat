...
then
	...
	$THIS_CISIS/mx seq=$DB.txt  create=$DB now -all
        $THIS_CISIS/mx $DB  fst=@$THIS_PATH/temp.fst fullinv=$DB now -all
	...
	$THIS_CISIS/mx $DB btell=0 "BREAK or VOL_BREAK" lw=9999 "pft=@$THIS_PATH/getA.pft" now > $myprocGetP.seq

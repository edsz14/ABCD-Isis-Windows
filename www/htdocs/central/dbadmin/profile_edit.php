...
//	echo "<xmp>".print_r($profile_usr)."</xmp>";//die;
	if ($profile!=""){
...
//	echo "<div style=\"position:relative;overflow:auto;height:300px;border-style:double;\">";
 	...
 	foreach($fp as $dbs){ 		$dbs=trim($dbs);
		if ($dbs!=""){
			$dd=explode('|',$dbs);
			$dbn=$dd[0];
			$select_db.= "<option value=".$dbn.">$dbn</option>\n";
		} 	}
 	$select_db.= "</select>\n";
	foreach ($fp as $dbs){
		$dbs=trim($dbs);
...
//	echo "</div>";
	...
	foreach ($general as $key){
		echo "<P><hr size=3><p>";
		echo "<table width=100%><tr height=5><th valign=top colspan=3 bgcolor=darkred >\n";
		...
		foreach ($profile_general[$modulo] as $usr_p=>$val){
			...
			$j=1;
			$checked="";
			if (isset($profile_usr[$modulo."_".$usr_p]) and $profile_usr[$modulo."_".$usr_p]=="Y") $checked=" checked";
			echo "<td width=33%><input type=checkbox name=$modulo"."_".$usr_p." $checked $onclick value=Y>".$msgstr[$usr_p]."</td>\n";
			$onclick="";
			if ($i>2){
				$i=0;
				echo "</tr>";

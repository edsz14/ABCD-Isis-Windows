...
if($_POST['cisis']!='')
$converter_path.=$_POST['cisis']."/wxis";
else
$converter_path.="wxis";
}
else
...
$converter_path.="wxis.exe";
}

$Wxis=$converter_path;
  
  @ $fp = fopen($db_path."abcd.def", "a");
@  flock($fp, 2);
  if (!$fp)
  {
...
@  flock($fp, 2);
 ...
 fwrite($fp, $str);
  ...
  if($_POST['dcimport']=='yes'&& $_POST['cisis']=='')
  {
  @ $fp = fopen($db_path.$arrHttp['base'].'/dr_path.def', "w");
@  flock($fp, 2);
  if (!$fp)
  ...
  {
  @ $fp = fopen($db_path.$arrHttp['base'].'/'.$arrHttp['base'].".par", "a");
@  flock($fp, 2);
  if (!$fp)
  {
    echo "<p><strong> Error ocurred in ISIS Script."
  ...
  $str="isisac.tab=".$db_path."www/isisactab_utf8.tab\n"."isisuc.tab=".$db_path."www/isisuctab_utf8.tab\n";
 fwrite($fp, $str);
  flock($fp, 3);
  fclose($fp);
  }

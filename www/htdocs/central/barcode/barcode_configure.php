...
function Actualizar(){
	if (Trim(document.actualizar.width.value)=="" || Trim(document.actualizar.height.value)=="" ||
	    Trim(document.actualizar.cols.value)==""  || Trim(document.actualizar.font_name.value)=="" ||
	    Trim(document.actualizar.font_size.value)=="" ){
...
echo "<font color=white>&nbsp; &nbsp; Script: barcode/barcode_configure.php";
?>
</font>
</div>
...
<form name=actualizar method=post action=barcode_configure_ex.php onsubmit="javascript:return false">
 ...
 <table>
 	<tr>
 		<td>Prefijo para extraer el código de barras desde la base de datos</td>
 		<td><input type=text name=prefijo size=10 value=<?php if (isset($parms["prefijo"])) echo $parms["prefijo"]?>></td>
 	</tr>
 	<tr>
 	...
 	<tr>
 		...
 		<td><input type=text name=font_name size=30 value="<?php if (isset($parms["font_name"])) echo $parms["font_name"]?>"></td>
 	</tr>
 	<tr>
 		<td>Tamaño de la fuente</td>
 		<td><input type=text name=font_size size=10 value=<?php if (isset($parms["font_size"])) echo $parms["font_size"]?>></td>
 	</tr>
    <tr>
    </table>
	<p>

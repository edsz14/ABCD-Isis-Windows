...
//CHECK IF THERE IS A VALIDATION FORMAT
	 	...
	 	switch ($arrHttp["Opcion"]){
			...
			case "dup_record":
				...
				if ((isset($_SESSION["permiso"]["CENTRAL_ALL"]) or isset($_SESSION["permiso"]["CENTRAL_BARCODE"]) or
	    			...
	    			and isset($_SESSION["BARCODE_SIMPLE"])){
    ...
    if (isset($arrHttp["Expresion"]) and $arrHttp["Expresion"]!=""){
    	...
    	echo "<div id=Expresion style=\"display:none; hide:block\"><textarea name=nueva_b cols=150 rows=1>".stripslashes($arrHttp["Expresion"])."</textarea> <a href=javascript:NuevaBusqueda()>".$msgstr["buscar"]."</a> <a href=javascript:top.Menu('refinar')>". $msgstr["refine"]."</a></font>";

...
function ReadCatalographicRecord($control_number,$Opcion,$db,$prefix_in,$tit_tabla,$Pft){
...
foreach ($codigos as $cod_inv){
	...
	if (strpos($lista_control_no,";".$cod_inv.";")!==false)
		continue;
	if ($Opcion=="inventario"){		$ejemp=ReadCatalographicRecord($cod_inv,$Opcion,$catalog_db,$pref_in,$tit_tabla,$Pft);
		$pref=$pref_in;
	}else{		$ejemp=ReadCatalographicRecord($cod_inv,$Opcion,$catalog_db,$pref_cn,$tit_tabla,$Pft);
		$pref=$pref_cn;	}
	echo "</table>";
	if (!isset($reserve_active) or isset($reserve_active) and $reserve_active=="Y"){
		...
		$reserves_arr=ReservesRead("CN_".$catalog_db."_".$control_no." and ST_3");
		$reserves_user=$reserves_arr[0];
		if ($reserves_user!="")
			echo $reserves_user."<p>";
		$control_no=ReadControlNumber($cod_inv,$Opcion,$catalog_db,$pref,$pft_nc);
		$reserves_arr=ReservesRead("CN_".$catalog_db."_".$control_no." and ST_0");
		$reserves_user=$reserves_arr[0];
		if ($reserves_user!="")
			echo $reserves_user."<p>";
		echo "<p>";

...
<c:choose>
  ...
  <c:when test="${empty param.library}">
    ...
    <h1><fmt:message key="historic_loans_by_record"/></h1>
    <h2><fmt:message key="historic_loans_by_record"/></h2>
    <form method="get">
    ...
    <c:choose>
      <c:when test="${param.printable eq 'true'}">
        <rep:requestStat name="hist-loans"
          ...
          <param name="searchField">recordId</param>
          <c:if test="${param.object_db ne '*'}">
            <param name="objectDb">${param.object_db}</param>
          </c:if>
          <param name="searchValue">${param.record_id}</param>
        </rep:requestStat>
      </c:when>
      <c:otherwise>
        <rep:requestStat name="hist-loans"

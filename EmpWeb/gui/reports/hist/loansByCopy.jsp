...
<c:choose>
  ...
  <c:when test="${empty param.library}">
    ...
    <rep:flushCache name="hist-loans"/>
    ...
    <h1><fmt:message key="historic_loans_by_copy"/></h1>
    <h2><fmt:message key="historic_loans_by_copy"/></h2>
    <form method="get">
      ...
      <table>
        <tr>
          ...
          <td><input type="text" name="copy_id"/></td>
        </tr>
        <tr>
          <td><fmt:message key="object_db"/></td>
          <td><dsp:selectObjectDatabase name="object_db" dbNames="${dbNames}" selectedDb="${objectDb}"/></td>
        </tr>
        <tr>
  ...
  <c:otherwise>
    ...
    <c:choose>
      <c:when test="${param.printable eq 'true'}">
        <rep:requestStat name="hist-loans"
          ...
          <param name="searchField">copyId</param>
          <c:if test="${param.object_db ne '*'}">
            <param name="objectDb">${param.object_db}</param>
          </c:if>
          <param name="searchValue">${param.copy_id}</param>
        </rep:requestStat>
      </c:when>
      <c:otherwise>
        <rep:requestStat name="hist-loans"
    ...
    <%-- THE CONTENT --%>
    <h1><fmt:message key="loans_list"/></h1>
    <h2><fmt:message key="loans_list"/>: ${param.library}</h2>
    ...
    <c:choose>
      <c:when test="${totalCount gt 0}">
        ...
        <table id="result">
          <%-- HEADERS --%>
          <tr><dsp:loanRow isheader="true" showsortlinks="true" urlprefix="${urlprefix}"/></tr>

          <%-- CONTENT ROWS --%>
          <c:forEach items="${sortedMap}" var="rowMap">
            <tr><dsp:loanRow rowMap="${rowMap}"/></tr>
          </c:forEach>
        </table>

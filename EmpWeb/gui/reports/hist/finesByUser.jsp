...
<c:choose>
  ...
  <c:when test="${empty param.library}">
    ...
    <rep:flushCache name="hist-fines"/>
    ...
    <h1><fmt:message key="historic_fines_by_user"/></h1>
    <h2><fmt:message key="historic_fines_by_user"/></h2>
    <form method="get">
      ...
      <table>
        ...
        <c:if test="${not config['ew.hide_user_db']}">
          <tr>
            ...
            <td><dsp:selectUserDatabase name="user_db" dbNames="${dbNames}"/></td>
          </tr>
        </c:if>
        <tr>
        ...
        <tr>
          <td></td>
          <td><input type="submit" name="submit" value="<fmt:message key='list_fines'/>"/></td>
        </tr>
      </table>
    </form>
  </c:when>
  ...
  <c:otherwise>
    ...
    <h2>
      <fmt:message key="historic_fines_by_user"/>:${param.user_id} (${param.user_db})
    ...
    <c:choose>
      <c:when test="${totalCount gt 0}">
        ...
        <%-- obtain the amount already paid for each fine --%>
        <c:forEach items="${sortedMap}" var="rowMap">
          <c:set var="fineId">${rowMap["transactionId"]}</c:set>
          
          <%-- get the payment details for this fine --%>
          <x:parse varDom="fineDetails">
            <trans:doTransaction name="stat-hist-fines">
              <transactionExtras>
                <params>
                  <param name="searchField">refId</param>
                  <param name="searchValue">${fineId}</param>
                </params>
              </transactionExtras>
            </trans:doTransaction>
          </x:parse>

          <%-- now extract the paymentsTotal and cancellation, and put it into the rowMap--%>
          <jxp:set cnode="${fineDetails}" var="paymentsTotal" select="//tr:value/tr:paymentsTotal" nsmap="${nsm}" />
          <c:set target="${rowMap}" property="paidAmount" value="${paymentsTotal}"/>
          <jxp:set cnode="${fineDetails}" var="cancelled" select="//tr:value/tr:cancelled" nsmap="${nsm}" />
          <jxp:set cnode="${fineDetails}" var="active" select="//tr:value/tr:active" nsmap="${nsm}" />
          <c:choose>
            <c:when test="${(cancelled eq 'true')}">
              <c:set target="${rowMap}" property="status" value="${'cancelled'}"/>
            </c:when>
            <c:when test="${(active eq 'true')}">
              <c:set target="${rowMap}" property="status" value="${'active'}"/>
            </c:when>
            <c:otherwise>
              <c:set target="${rowMap}" property="status" value="${'paid'}"/>
            </c:otherwise>
          </c:choose>
        </c:forEach>

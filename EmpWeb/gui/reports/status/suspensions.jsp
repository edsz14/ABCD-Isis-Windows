...
<c:choose>
  ...
  <c:when test="${empty param.library}">    
    ...
    <rep:flushCache name="stat-suspensions"/>
    ...
    <h1><fmt:message key="suspensions_list_parameters"/></h1>
    <h2><fmt:message key="suspensions_list_parameters"/></h2>  
    <form method="get">
  ...
  <c:otherwise>
    ...
    <c:choose>
      <c:when test="${param.printable eq 'true'}">
        <rep:requestStat name="status-suspensions" 
    ...
    <%-- THE CONTENT --%>
    <h1><fmt:message key="active_suspensions_list"/></h1>
    <h2><fmt:message key="active_suspensions_list"/>: ${param.library}</h2>

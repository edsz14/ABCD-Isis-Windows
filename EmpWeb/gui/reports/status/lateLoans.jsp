...
<c:choose>
  ...
  <c:when test="${empty param.library}">
    ...
    <rep:flushCache name="stat-lateLoans"/>
    ...
    <h1><fmt:message key="late_loans_list_parameters"/></h1>
    <h2><fmt:message key="late_loans_list_parameters"/></h2>
    <form method="get">
  ...
  <c:otherwise>
    ...
    <c:choose>
      <c:when test="${param.printable eq 'true'}">
        <rep:requestStat name="status-lateLoans"
    ...
    <%-- THE CONTENT --%>
    <h1><fmt:message key="late_loans_list"/></h1>
    <h2><fmt:message key="late_loans_list"/>: ${param.library}</h2>

...
<c:choose>
  ...
  <c:when test="${empty param.library}">    
    ...
    <rep:flushCache name="stat-loans"/>
    ...
    <h1><fmt:message key="loans_waits_parameters"/></h1>
    <h2><fmt:message key="loans_waits_parameters"/></h2>  
    <form method="get">
      <table>
  ...
  <c:otherwise>
    ...
    <c:choose>
      <c:when test="${param.printable eq 'true'}">
        <rep:requestStatByRecord name="status-waits-assigned" 
    ...
    <%-- THE CONTENT --%>
    <h1><fmt:message key="waits_list"/></h1>
    <h2><fmt:message key="waits_list"/>: ${param.library}</h2>
    ...
    <c:choose>
      <c:when test="${totalCount gt 0}">
        ...
        <table id="result">
          <%-- HEADERS --%>
          <tr><dsp:waitRow isheader="true" showsortlinks="true" urlprefix="${urlprefix}"/></tr>
          
          <%-- CONTENT ROWS --%>
          <c:forEach items="${sortedMap}" var="rowMap">
            <tr><dsp:waitRow rowMap="${rowMap}"/></tr>
          </c:forEach>
        </table>

...
<c:if test="${isCached eq 'false'}">
  ...
  <c:if test="${(not empty fillObjectsInfo) and (fillObjectsInfo eq 'true')}">
    <jsp:useBean id="uniqueCopyIds" class="java.util.HashMap" />
    <jxp:forEach cnode="${transactions}" var="ptr" select="//*[local-name()='copyId']" nsmap="${nsm}">
      <c:set target="${uniqueCopyIds}" property="${ptr['.']}," value="${ptr['.']}," />
    </jxp:forEach>

    <x:parse varDom="objectsInfo">
      <trans:searchObjectsById database="*">
        <c:forEach items="${uniqueCopyIds}" var="_cpid">${_cpid.value}</c:forEach>
...
while(it.hasNext())
  {
    ...
    String transactionHistoric=   n.getAttributes().getNamedItem("historic").getNodeValue();
    ...
    hm.put("transactionHistoric", transactionHistoric);
